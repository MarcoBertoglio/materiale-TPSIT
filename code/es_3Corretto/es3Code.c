...
void push (struct El **head, struct El** tail, struct El *element){
    ...
    while (!is_empty(*head)){
        element2 = dequeue(head,tail); //toglie l'elemento della coda pricipale
        enqueue(&supportHead, &supportTail, element2); //metto l'elemento nella coda di supporto
    }

    enqueue(head, tail, element);

    while (!is_empty(supportHead)){ //ciclo sulla coda d'appoggio
        enqueue(head, tail, dequeue(&supportHead, &supportTail)); //mette gli elementi della coda d'appoggio in quella
                                                                  //pricipale
    }
}
